...
describe('<TimelineEventModal />', () => {
  context(
    'when passed in a data object with appName, timestamp, and body',
    () => {
      const data = {
        appName: 'side effect',
        timestamp: '5:51 PM',
      ...
      it('the app name correctly renders', () => {
        ...
        expect(timelineEventModal
          .find('#appName')
          .first()
          .children()).to.contain(data.appName);
      });
      ...
      it('the timestamp correctly renders', () => {
        ...
        expect(timelineEventModal
          .find('#timestamp')
          .first()
          .children()).to.contain(data.timestamp);
      });
